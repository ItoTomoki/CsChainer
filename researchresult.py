...
def createConpArray2(testdata,preddata):
	...
	for i in range(len(testdata)):
		...
		for k in range(len(g)-1):
			if g[k] == g[k + 1]:
				sameTestarray.append(g[k])
				#sameTestarray.append(g[k+1])
				samePredarray.append(g2[k])
			else:
				defferentaTestrray.append(g[k])
				#defferentaTestrray.append(g[k+1])
				defferentaPredarray.append(g2[k])
	return np.array(sameTestarray),np.array(defferentaTestrray),np.array(samePredarray),np.array(defferentaPredarray)

sameTestarray,defferentaTestrray,samePredarray,defferentaPredarray = createConpArray2(testdammydata,preddamydata)
print accuracy_score(np.array(defferentaTestrray),np.array(defferentaPredarray))
print accuracy_score(np.array(sameTestarray),np.array(samePredarray))
