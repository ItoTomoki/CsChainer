...
K = 0
for sentencelist in sentenceslist_file:
	K += len(sentencelist)
...
def forward(src_sentences, trg_sentences, model, training):
	...
	# yの中で最大の確率を持つ単語を選択していくが、softmaxを取る必要はない。
		while len(hyp_sentences) < 10: #10文以上は生成しないようにする
			q = model.w_Qq(Q)
			j = tanh(model.w_qj(q))
			#j = tanh(model.w_qj(Q))
			y = model.w_jy(j)
...
forward(sentenceslist_file[1][2], sentenceslist_file[1][2], model, training = False)
N = 1000
...
for i in range(0,200):
	...
	hyp_sentence = forward(sentenceslist_file[1][0],sentenceslist_file[1][0],model, training = True)
